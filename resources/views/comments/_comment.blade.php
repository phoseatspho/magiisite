        ...
        <div class="media-body row mw-100 mx-0" style="flex:1;flex-wrap:wrap;">
            ...
            @if (isset($compact) && !$compact)
                <div class="d-none d-md-block">
                    <img class="mr-3 mt-2" src=" {!!  $comment->commenter->userBorder !!}" style="width:70px; height:70px; border-radius:50%;" alt=" {!!  $comment->commenter->userBorder !!} Avatar">
            ...
            <div class="d-block" style="flex:1">
                ...
                <div class="row mx-0 px-0 align-items-md-end">
                    <h5 class="mt-0 mb-1 col mx-0 px-0">
                        {!! $comment->commenter->commentDisplayName !!} @if ($comment->commenter->isStaff == true)
                            <small class="text-primary">Staff Member</small>
                        @endif
                    </h5>
                ...
                {{-- Comment --}}
                <div
                    class="site-comment border p-3 rounded {{ $comment->is_featured ? 'border-success bg-light' : '' }} {{ $comment->likes()->where('is_like', 1)->count() -$comment->likes()->where('is_like', 0)->count() <0? 'bg-light bg-gradient': '' }}">
